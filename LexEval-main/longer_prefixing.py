...
def get_answer(index, og_index, device, generator, modelId, embedder):
    ...
    checked_tree_file_path = f"{checked_tree_dir}/{modelId.replace('/', '-')}/complete/{og_index}_checked.pkl"
        ...
        try:
            if not os.path.exists(checked_tree_file_path):
                test_tree = Tree.load_tree(
                    device, tree_file_path, eval="eval", generator=generator, embedder=embedder
                )
...
def parse_args():
    ...
    parser.add_argument("--gen_modelId", type=str, default="all", help="Generation model ID (e.g., google/gemma-3-12b-it)")
...
def main():
    # ###################### parse input ######################
    global strategy, gen_modelIds, eval_only
    args = parse_args()
    # prefix, paraphrase, paraphrase_then_prefix
    strategy = args.strategy
    gen_modelIds = constants.MODELS if all else args.gen_modelId
    eval_only = args.eval_only
    start_idx = args.start_idx
    end_idx = args.end_idx
    dataset_name = args.dataset_name
    update_params(dataset_name)
     
    formatted_time = datetime.fromtimestamp(time.time()).strftime("%m-%d-%H:%M")
    filename = f"{constants.LOG_DIR}{formatted_time}_{start_idx}_to_{end_idx}_{statrgy_path}.log"
    configure_logging(filename)

    # create dlq file
    dlq_path = f"{constants.DLQ_DIR}{formatted_time}_dlq.log"
    ...
    logging.info(f"answer generator: {gen_modelIds}")
